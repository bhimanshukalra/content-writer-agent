...
function showRouting(event) {
  ...
  if (Array.isArray(event.queries) && event.queries.length) {
    ...
    if (element) {
      element.querySelector(".timeline-content").appendChild(queriesElement);
    }
  }
}
...
function showPlan(plan) {
  ...
  planMeta.innerHTML = `
        ...
        <div>
            ...
            <strong>
                ${escapeHtml(
                  (plan.blog_kind || "explainer").replaceAll("_", " "),
                )}
            </strong>
        </div>
...
function handleEvent(event) {
  switch (event.type) {
    ...
    case "images_planned":
      showToast(`${event.count} visual${event.count === 1 ? "" : "s"} planned`);
      break;
